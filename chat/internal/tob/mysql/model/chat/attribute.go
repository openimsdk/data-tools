...
import (
	...
	"github.com/openimsdk/data-tools/chat/internal/tob/mysql/table/chat"
...
func (o *Attribute) SearchUserID(ctx context.Context, keyword string, userIDs []string, pagination pagination.Pagination) (int64, []string, error) {
	...
	if keyword != "" {
		filter["$or"] = []bson.M{
			...
			{"english_name": bson.M{"$regex": keyword, "$options": "i"}},
			{"station": bson.M{"$regex": keyword, "$options": "i"}},
			{"telephone": bson.M{"$regex": keyword, "$options": "i"}},
		}
	}
	return mgoutil.FindPage[string](ctx, o.coll, filter, pagination, options.Find().SetProjection(bson.M{"_id": 0, "user_id": 1}))
}
